...
void SyntheticTurbulence::CopyToGPU(STWaveSet& ST) {
    ...
    if (ST.nmodes != 0) { CudaMemcpy(ST.data, cpuset.data, sizeof(real_t)*ST.nmodes*ST_DATA, CudaMemcpyHostToDevice); }
}

void runif(int n, double* tab) {
    for (int i=0; i<n; i++) tab[i] = (double)rand()/RAND_MAX;
}
...
void SyntheticTurbulence::Generate() {
    ...
    for (int j=0; j<size; j++) {
        if (D_MPI_RANK == 0) {
            ...
            for (int i=0; i<6; i++) cpuset.data[j*ST_DATA + i] = tab[i];
...
void SyntheticTurbulence::setVonKarman(double Le, double Ld, double Lmin, double Lmax) {
    ...
    if (sum < 0.7) NOTICE("Total energy of synthetic turbulence is below 70%% of the spectrum\n");
        ...
        notice("Total energy of synthetic turbulence is below 80%% of the spectrum\n");
    else if (sum > 1)
        NOTICE("Total energy of synthetic turbulence is above 100%% of the spectrum\n");
