...
struct CrossOPAdd {
    template <class T>
    __device__ static inline T op(const T& x, const T& y) {
        return x + y;
    }
};
struct CrossOPMax {
...
__device__ inline void CudaAtomicAddReduce(real_t* sum, real_t val) {
    ...
    if (j == 0) {
        real_t val = sumtab[0];
        if (val != 0.0) { CudaAtomicAdd(sum, val); }
    }
}
...
__device__ inline void CudaAtomicMaxReduce(real_t* sum, real_t val) {
    ...
    if (j == 0) {
        real_t val = sumtab[0];
        if (val != 0.0) { CudaAtomicMax(sum, val); }
    }
}
...
#define FULL_MASK 0xffffffff
    if (__any_sync(FULL_MASK, val != 0)) {
        for (int offset = WARPSIZE/2; offset > 0; offset /= 2) val += __shfl_down_sync(FULL_MASK, val, offset, WARPSIZE);
...
#define FULL_MASK 0xffffffff
    if (__any_sync(FULL_MASK, val != 0)) {
        for (int offset = WARPSIZE/2; offset > 0; offset /= 2) val = max(val, __shfl_down_sync(FULL_MASK, val, offset, WARPSIZE));
...
#define FULL_MASK 0xffffffff
    ...
    if (__any_sync(FULL_MASK, pred)) {
        ...
        if (threadIdx.x < LEN) { CudaAtomicAdd(sum + threadIdx.x, val[threadIdx.x]); }
    }
}
...
__device__ inline void CudaAtomicAddReduceWarp(real_t* sum, real_t val) {
    if (__any(val != 0)) {
        for (int offset = WARPSIZE/2; offset > 0; offset /= 2) val += __shfl_down(val, offset, WARPSIZE);
        if (threadIdx.x == 0) CudaAtomicAdd(sum, val);
    }
}
...
__device__ inline void CudaAtomicMaxReduceWarp(real_t* sum, real_t val) {
    if (__any(val != 0)) {
        for (int offset = WARPSIZE/2; offset > 0; offset /= 2) val = max(val, __shfl_down(val, offset, WARPSIZE));
        if (threadIdx.x == 0) CudaAtomicMax(sum, val);
    }
}
