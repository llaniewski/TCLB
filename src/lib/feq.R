...
MRT_polyMatrix <- function(U) {
    ...
    W <- NULL
    for (i in 1:nrow(p)) {
        W <- cbind(W, apply(t(U)^p[i, ], 2, prod))
    }
...
WMRT_mat <- function(U) {
    get.M.matrix <- function(a) {
        ...
        an <- setdiff(an, ".M")
        ret <- lapply(a@vec, function(p) {
            nan <- setdiff(an, names(p))
            p[, nan] <- 0
            p
        })
        n <- length(ret)
