...
template <typename T>
MPI_Datatype MPI_Auto_datatype();
template <>
inline MPI_Datatype MPI_Auto_datatype<int>() {
    return MPI_INT;
}
template <>
inline MPI_Datatype MPI_Auto_datatype<unsigned int>() {
    return MPI_UNSIGNED;
}
template <>
inline MPI_Datatype MPI_Auto_datatype<long int>() {
    return MPI_LONG;
}
template <>
inline MPI_Datatype MPI_Auto_datatype<unsigned long int>() {
    return MPI_UNSIGNED_LONG;
}
template <>
inline MPI_Datatype MPI_Auto_datatype<char>() {
    return MPI_CHAR;
}

template <class T>
T MPI_Exchange(T& out, MPI_Comm intercomm, MPI_Comm comm) {
    T ret;
...
public:
    ...
    void Init(const MPI_Comm& world_, const std::string& name_, std::vector<int> excl = std::vector<int>()) {
        ...
        if (parent_ != MPI_COMM_NULL) { parent = ConnectIntercomm(parent_, false); }
    }
    ...
    void Identify(bool world_ = false) {
        ...
        iden += ":";
        if (world_) {
            sprintf(iden_c, " world:%d/%d", world_rank, world_size);
            iden += iden_c;  // LCOV_EXCL_LINE
        }
        sprintf(iden_c, " local:%d/%d", local_rank, local_size);
        iden += iden_c;
        if (in_work) {
        ...
        for (intercomm_map::iterator c = intercomm.begin(); c != intercomm.end(); c++) {
            ...
            iden += "(";
            if (inter.in_work) {
                sprintf(iden_c, "%d", inter.work_size);
                iden += iden_c;
            } else
                iden += "NA";
            iden += "/";
            if (inter.in_local) {
                sprintf(iden_c, "%d", inter.local_size);
                iden += iden_c;
            } else
                iden += "NA";
            iden += ")";
