...
struct GPUAnimBitmap {
    ...
    void mouse_move(void (*f)(void*, int, int, int, int)) { move = f; }
    ...
    void anim_and_exit(int (*f)(uchar4*, void*, int), void (*e)(void*)) {
        ...
        *bitmap = this;
        ...
        if (clickDrag != NULL || move != NULL) glutMouseFunc(mouse_func);
        if (move != NULL) glutMotionFunc(move_func);
    ...
    static void mouse_func(int button, int state, int mx, int my) {
        if (button == GLUT_LEFT_BUTTON) {
            ...
            if (state == GLUT_DOWN) {
                ...
                if (bitmap->clickDrag != NULL) { bitmap->clickDrag(bitmap->dataBlock, bitmap->dragStartX, bitmap->dragStartY, mx, my); }
                if (bitmap->move != NULL) { bitmap->move(bitmap->dataBlock, bitmap->dragStartX, bitmap->dragStartY, mx, my); }
                bitmap->dragStartX = mx;
                bitmap->dragStartY = my;
            }
        }
    ...
    static void move_func(int mx, int my) {
        ...
        bitmap->move(bitmap->dataBlock, bitmap->dragStartX, bitmap->dragStartY, mx, my);
        bitmap->dragStartX = mx;
        bitmap->dragStartY = my;
    }
