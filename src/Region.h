...
struct lbRegion {
    ...
    int isEqual(const lbRegion& other) const {
        return (other.dx == dx) && (other.dy == dy) && (other.dz == dz) && (other.nx == nx) && (other.ny == ny) && (other.nz == nz);
    }
    ...
    int offset(int x, int y) const { return (x - dx) + (y - dy)*nx; };
    void print() const { printf("Region: %dx%dx%d + %d,%d,%d\n", nx, ny, nz, dx, dy, dz); };
    CudaHostFunction CudaDeviceFunction int offset(int x, int y, int z) const { return (x - dx) + (y - dy)*nx + (z - dz)*nx*ny; };
    CudaHostFunction CudaDeviceFunction size_t offsetL(int x, int y, int z) const { return (x - dx) + (y - dy)*nx + (z - dz)*nx*ny; };
};
