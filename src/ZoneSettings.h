...
public:
    ...
    inline ZoneSettings(int zonesettings_, int zones_) : zonesettings(zonesettings_), zones(zones_) {
        ...
        for (int i = 0; i < time_seg(); i++) { cpuConst[i] = 0.0; }
        DEBUG_M;
    ...
    inline void set_internal(int i, const double* val) {
        Alloc(i);
        for (size_t j=0; j<len; j++) { cpuValues[i][j] = val[j]; }
        Alloc(i + dt_offset());
        for (size_t j = 1; j < len - 1; j++) { cpuValues[i + dt_offset()][j] = (val[j + 1] - val[j - 1])/2; }
    ...
    inline void get(int s, int z, double* tab) {
        ...
        if (cpuValues[i] == NULL) {
            ...
            for (size_t it=0; it<len; it++) { tab[it] = cpuValues[i][it]; }
        }
    }
    ...
    inline void ClearGrad() {
        for (int i = grad_offset(); i < time_seg(); i++)
            if (cpuValues[i] != NULL) {
