...
public:
    void* do_allocate(size_t bytes, size_t alignment) override {
        ...
        if (aligned != cuda_ptr) ptr_map.emplace(aligned, cuda_ptr);
        return aligned;
    }
    ...
    bool do_is_equal(const std::pmr::memory_resource& other) const noexcept override {
        if (auto as_pinned_ptr = dynamic_cast<const PinnedMemoryResource*>(&other); as_pinned_ptr) return ptr_map == as_pinned_ptr->ptr_map;
        return false;
    }
};
