...
/// Globally, nodes are numbered such that consecutive ranks own monotonically increasing intervals, (e.g. rank 0 owns nodes [0, n_0), rank 1 owns nodes [n_0,
/// n_1), etc.) This is required for ParMETIS, but it is also a convenient scheme in general.
///     When communicating between ranks, only this numbering is used.
/// Locally, nodes are permuted such that:
///     - Owned boundary nodes (i.e. nodes which have ghost nodes among their neighbors) occupy the initial B indices, starting at 0. Their numbering within [0,
///     B) is arbitrary w.r.t. the global scheme
...
///     - Ghost nodes (i.e. nodes neighboring owned nodes, but not owned by the current rank) occupy indices [B + I, B + I + G), where G is the number of ghost
///     nodes. **Their numbering within that interval corresponds to the global numbering scheme (important for optimal packing)**
///     - There is at least one padding index B + I + G, corresponding to the row where dummy values (NaNs) will be stored for when a nonexistent neighbor is
///     accessed, more padding may be present to promote coalesced memory access
/// Note that the GPU is only aware of the local numbering scheme, which uses 32b indexing, saving memory bandwidth. Local indexing can additionally put
/// "similar" nodes next to each other, minimizing branching and promoting coalesced memory access. Naming: GID == global ID; LID == local ID
...
class ArbLattice : public LatticeBase {
    ...
    struct SizeInfo {
        ...
        size_t coords_pitch;     /// B + I + padding (should be the same as neighbors_pitch, but let's be extra safe since they come from separate pitched
                                 /// allocation calls)
...
public:
    /// Struct for storing arb lattice geometry info for export to VTU (ParaView unstructured grid format). Note that the ordering is permuted, so that solution
    /// data can be directly written to the results file
    struct ArbVTUGeom {
...
private:
    ...
    std::vector<long>
        global_node_dist;  /// Node distribution (in the ParMETIS sense), describing the GID node intervals owned by each process (identical in all ranks)
    ...
    lbRegion local_bounding_box;  /// The bounding box of the local region (if the arbitrary lattice is a subset of a full Cartesian lattice, we can use this
    ...
    pugi::xml_node
        initialized_from;    /// XML node from which this node was initialized - avoid reinitialization if called multiple times with the same arguments
...
public:
    ...
    int reinitialize(
        size_t num_snaps_,
        const std::map<std::string, int>& setting_zones,
        pugi::xml_node arb_node);  /// Init if passed args differ from those passed at construction or the last call to reinitialize (avoid duplicating work)
...
#ifdef ADJOINT
    void setAdjSnapIn(int tab) {
        launcher.container.adj_snap_in = getAdjointSnapPtr(tab);
    }
    void setAdjSnapOut(int tab) {
        launcher.container.adj_snap_out = getAdjointSnapPtr(tab);
    }
#endif
    void MPIStream_A();
    void MPIStream_B();
...
private:
    struct NodeTypeBrush {
        std::function<bool(Span<const ArbLatticeConnectivity::ZoneIndex>, std::array<double, 3>)>
            pred;            /// Predicate to determine whether the brush is applicable to this node, takes the labels and coordinates of the node
    ...
    const storage_t* getSnapPtr(int snap_ind) const {
        return const_cast<ArbLattice*>(this)->getSnapPtr(snap_ind);
    }
#ifdef ADJOINT
    ...
    std::function<bool(int, int)> makePermCompare(
        pugi::xml_node arb_node,
        const std::map<std::string, int>&
            setting_zones);  /// Make type-erased comparison operator for computing the local permutation, according to the strategy specified in the xml file
    void computeLocalPermutation(pugi::xml_node arb_node,
                                 const std::map<std::string, int>& setting_zones);  /// Compute the local permutation, see comment at the top
    ...
    std::vector<NodeTypeBrush> parseBrushFromXml(pugi::xml_node arb_node, const std::map<std::string, int>& setting_zones)
        const;  /// Parse the arbitrary lattice XML to determine the brush sequence to be applied to each node
    void computeNodeTypesOnHost(pugi::xml_node arb_node,
                                const std::map<std::string, int>& setting_zones,
                                bool permute);            /// Compute the node types to be stored on the device, `permute` enables better code reuse
    ...
    int fullLatticePos(
        double pos) const;  /// Compute the position (in terms of lattice offsets) of a node, assuming the arbitrary lattice is a subset of a Cartesian lattice
