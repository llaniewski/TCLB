...
struct base64decoder {
    ...
    int rev64[256];
    base64decoder() {
        for (int i=0; i<256; i++) rev64[i] = 666;
        for (int i=0; i<64; i++) rev64[((unsigned char*)base64char)[i]] = i;
...
struct TabBase {
    ...
    size_t idx(const int& x, const int& y, const int& z, const int& c) const { return c + comp*(x - dx + nx*(y - dy + ny*(z - dz + 0L))); }
...
struct Tabs {
    ...
    Tabs(std::string filename_) : filename(filename_) {
        ...
        printf("\n");
        for (pugi::xml_node it = el.child("Piece"); it; it = it.next_sibling("Piece")) { read_piece(it); }
    }
...
int main(int argc, char* argv[]) {
    ...
    switch (argc) {
        case 7:
            sscanf(argv[6], "%d", &delta_z);
        case 6:
            sscanf(argv[5], "%d", &delta_y);
        case 5:
            sscanf(argv[4], "%d", &delta_x);
        case 4:
    ...
    for (std::set<std::string>::iterator it = names.begin(); it != names.end(); it++) {
        ...
        if (tabs1.tab.find(name) == tabs1.tab.end()) {
            ...
            if (auto_eps != 0) { printf(" = %.1lf*%lg", diff/auto_eps, auto_eps); }
