...
void Solver::Gauge() {
    units.makeGauge();
    if (mpi_rank == 0) { units.printGauge(); }
}
...
void Solver::setOutput(std::string out) {
    ...
    if (lattice) lattice->snapFileName = std::move(out) + std::move(conffile_stem) + "_Snap";
...
int Solver::initLog(const char* filename) {
    if (mpi_rank == 0) {
        ...
        for (auto v : lattice->model->zonesettings) {
            for (const auto& [name, id] : setting_zones) { fprintf(f, ",\"%s-%s\",\"%s-%s_si\"", v.name.c_str(), name.c_str(), v.name.c_str(), name.c_str()); }
            LogScales.push_back(1/units.alt(v.unit));
        }
...
int Solver::writeLog(const char* filename) {
    ...
    if (mpi_rank == 0) {
        ...
        assert(f != NULL);
        int j =
            lattice->model->settings.size() + lattice->model->zonesettings.size() + lattice->model->globals.size() + lattice->model->settings.by_name("dt").id;
...
static CartConnectivity makeConnectivity(int nx, int ny, int nz, MPI_Comm comm) {
    ...
    CartConnectivity retval;
    if (my_rank == 0) retval = partitionCartLattice(nx, ny, nz, comm_sz);
    else
