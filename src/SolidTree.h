...
public:
        ...
        class iterator_t {
            ...
            CudaDeviceFunction void go(bool go_left) {
                while (nodei != -1) {
                    ...
                    int dir = elem.flag;
                    if (go_left)
                        if (set->lower[dir] < elem.b) {
                            nodei++;
                            continue;
                        }
                    go_left = true;
                    if (set->upper[dir] >= elem.a) {
                        nodei = elem.right;
                        continue;
                    }
                    go_left = false;
            ...
            CudaDeviceFunction iterator_t(const set_found_t& set_) : set(&set_) {
                nodei = 0;
                if (set->finder.data_size == 0) {
                    nodei = -1;
                    return;
                }
                go(true);
            };
            CudaDeviceFunction iterator_t() : set(nullptr) {
                nodei = -1;
                return;
            };
    ...
    public:
        ...
        CudaDeviceFunction inline set_found_t(
            const finder_t& finder_, real_t* particle_data_, const real_t point_[], const real_t lower_[], const real_t upper_[])
            : finder(finder_), particle_data(particle_data_) {
            for (int i=0; i<3; i++) {
                lower[i] = lower_[i];
                upper[i] = upper_[i];
                point[i] = point_[i];
    ...
    public:
        ...
        CudaDeviceFunction inline cache_set_found_t(
            const finder_t& finder, real_t* particle_data_, const real_t point_[], const real_t lower[], const real_t upper[])
...
int SolidTree<BALLS>::half(int i, int j, int dir, tr_real_t thr) {
    if (i == (--j)) return i;
    while (true) {
        while (balls->getPos(nr[i], dir) <= thr)
            if ((++i) == j) return i;
        while (balls->getPos(nr[j], dir) > thr)
            if (i == (--j)) return i;
        int tmp = nr[i];
        nr[i] = nr[j];
        nr[j] = tmp;
...
tr_addr_t SolidTree<BALLS>::build(int ind, int n, int back) {
    ...
    if (n - ind < 2) {
        ...
        if (!(ind < d)) { printf("Something is wrong in ball tree build: %d = half(%d, %d, %d, %lg); // max_span = %lg\n", d, ind, n, dir, sum, max_span); }
        assert(ind < d);
        assert(d < n);
        {
        ...
        //        printf("%d %3d %3d %lg %lg %lg %d --- %lg (%2.0lf%% %2.0lf%%)\n", node, ind, n, sum, v_min, v_max, d, v_max-v_min,
        //        100.0*(d-ind)/(n-ind),100.0*(n-d)/(n-ind));
        build(ind, d, node);
...
void SolidTree<BALLS>::CopyToGPU(finder_t& finder, CudaStream_t stream) {
    ...
    if (tree.size() > 0) { CudaMemcpyAsync(finder.data, (tr_elem*)&tree[0], tree.size()*sizeof(tr_elem), CudaMemcpyHostToDevice, stream); }
}
