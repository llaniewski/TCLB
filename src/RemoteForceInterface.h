...
enum rfi_type_t { ForceCalculator, ForceIntegrator };
enum rfi_rot_t { RotParticle, NRotParticle };
enum rfi_storage_t { ArrayOfStructures, StructureOfArrays };

template <rfi_type_t TYPE,
          rfi_rot_t ROT,
          rfi_storage_t STORAGE = ArrayOfStructures,
          typename rfi_real_t = double,
          typename tab_allocator = std::allocator<rfi_real_t> >
...
public:
    ...
    template <class T>
    inline T Exchange(T out);
    template <class T>
    inline std::vector<T> Exchange(std::vector<T> out);
    template <class T>
    inline std::basic_string<T> Exchange(std::basic_string<T> out);
    ...
    inline rfi_real_t getPos(size_t i, int j) { return getData(i, RFI_DATA_POS + j); }
    inline rfi_real_t getRad(size_t i) { return getData(i, RFI_DATA_R); }
    inline void setData(size_t i, int j, rfi_real_t val) { RawData(i, j) = val*unit[j]; }
    inline rfi_real_t getData(size_t i, int j) { return RawData(i, j)/unit[j]; }
};
