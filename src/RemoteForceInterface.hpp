...
#define safe_MPI_Type_free(datatype)                      \
    {                                                     \
        if ((*datatype) != NULL) MPI_Type_free(datatype); \
    }

template <typename T>
inline MPI_Datatype MPI_dt();
template <>
inline MPI_Datatype MPI_dt<int>() {
    return MPI_INT;
}
template <>
inline MPI_Datatype MPI_dt<unsigned int>() {
    return MPI_UNSIGNED;
}
template <>
inline MPI_Datatype MPI_dt<long int>() {
    return MPI_LONG;
}
template <>
inline MPI_Datatype MPI_dt<unsigned long int>() {
    return MPI_UNSIGNED_LONG;
}
template <>
inline MPI_Datatype MPI_dt<char>() {
    return MPI_CHAR;
}
template <>
inline MPI_Datatype MPI_dt<float>() {
    return MPI_FLOAT;
}
template <>
inline MPI_Datatype MPI_dt<double>() {
    return MPI_DOUBLE;
}

template <rfi_type_t TYPE, rfi_rot_t ROT, rfi_storage_t STORAGE, typename rfi_real_t, typename tab_allocator>
template <class T>
inline T RemoteForceInterface<TYPE, ROT, STORAGE, rfi_real_t, tab_allocator>::Exchange(T out) {
    T in;
    MPI_Request request;
    MPI_Status status;
...
template <rfi_type_t TYPE, rfi_rot_t ROT, rfi_storage_t STORAGE, typename rfi_real_t, typename tab_allocator>
template <class T>
inline std::vector<T> RemoteForceInterface<TYPE, ROT, STORAGE, rfi_real_t, tab_allocator>::Exchange(std::vector<T> out) {
    std::vector<T> in;
...
template <rfi_type_t TYPE, rfi_rot_t ROT, rfi_storage_t STORAGE, typename rfi_real_t, typename tab_allocator>
template <class T>
inline std::basic_string<T> RemoteForceInterface<TYPE, ROT, STORAGE, rfi_real_t, tab_allocator>::Exchange(std::basic_string<T> out) {
...
void RemoteForceInterface<TYPE, ROT, STORAGE, rfi_real_t, tab_allocator>::MakeTypes(bool particle_size_change, bool totsize_change) {
    ...
    if (STORAGE == ArrayOfStructures) {
        ...
        if (totsize_change) {
            ...
            for (int i=0; i<RFI_DATA_SIZE; i++) {
                mt_offsets[i] = i*totsize;
                mt_sizes[i] = 1;
            }
...
int RemoteForceInterface<TYPE, ROT, STORAGE, rfi_real_t, tab_allocator>::Negotiate() {
    ...
    if (my_stats_prefix == "") { stats_prefix = other_stats_prefix; }

    int my_stats_iter = stats_iter;
    int other_stats_iter = Exchange(my_stats_iter);

    if (my_stats_iter == 0) { stats_iter = other_stats_iter; }
    ...
    if (TYPE == ForceCalculator) { ISendSizes(); }
...
template <rfi_type_t TYPE, rfi_rot_t ROT, rfi_storage_t STORAGE, typename rfi_real_t, typename tab_allocator>
void RemoteForceInterface<TYPE, ROT, STORAGE, rfi_real_t, tab_allocator>::DeclareSimpleBox(
    rfi_real_t x0, rfi_real_t x1, rfi_real_t y0, rfi_real_t y1, rfi_real_t z0, rfi_real_t z1) {
    myBox.declared = true;
