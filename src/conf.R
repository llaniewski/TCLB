...
if (plot_access) {
    ...
    pa_fi <- by(pa_fi, pa_fi$group, function(x) {
        x$groupsize <- nrow(x)
        x
    })
    ...
    if (length(a$stages) != 0) {
        ...
        if (plot_access) {
            ...
            legend(par("usr")[2], par("usr")[3],
                xpd = TRUE, yjust = 1, xjust = 1, ncol = 2, cex = 0.7, bty = "n", bg = "white",
        ...
        for (sn in a$stages) {
            ...
            if (plot_access) {
                ...
                sel <- pa_col != "white"
                if (any(sel)) segments(pa_a1x, pa_a1y[sel], pa_a2x, pa_a2y[sel], col = pa_col[sel])
...
tmp_c <- Globals[Globals$op != "SUM", , drop = FALSE]
tmp_c <- tmp_c[order(tmp_c$op), , drop = FALSE]
...
for (g in rows(Globals)[Globals$op == "SUM" & Globals$name != "Objective"]) {
    if (!g$adjoint) {
        AddSetting(
...
is.power.of.two <- function(x) {
    2^floor(log(x) / log(2)) - x != 0
}
        ...
        list(
            get_offsets =
                function(w, dw) {
                    ...
                    dw_neg <- sapply(1:3, function(i) {
                        if (is.numeric(dw[[i]])) dw[[i]] < 0 else mins[i] < 0
                    })
                    dw_pos <- sapply(1:3, function(i) {
                        if (is.numeric(dw[[i]])) dw[[i]] > 0 else maxs[i] > 0
                    })
...
git_version <- function() {
    f <- pipe("git describe --always --tags")
    v <- readLines(f)
    close(f)
    v
}
