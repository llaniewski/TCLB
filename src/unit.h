...
public:
    ...
    void print() const { output("%s\n", tmp_str()); };
    /// Convert to string
    std::string toString() const {
        std::string str = std::to_string(val);
        std::string div = "/";
        for (int i=0; i<m_unit; i++)
            if (uni[i] > 0) { str = str + m_units[i] + std::to_string(uni[i]); }
        for (int i=0; i<m_unit; i++)
            if (uni[i] < 0) {
...
public:
    ...
    double si(const std::string& str, double def) const {
        if (str.length() > 0) return si(str);
        else
            return def;
    }
    double alt(const std::string& str, double def) const {
        if (str.length() > 0) return alt(str);
        else
            return def;
    }
...
public:
    ...
    void operator=(const std::string& str) { *this = env->readText(str); }
    double si() { return env->si(*this); }
    double alt() { return env->alt(*this); }
};
