...
#define CudaKernelRun(a__, b__, c__, ...)  \
    a__<<<b__, c__>>>(__VA_ARGS__);        \
    HANDLE_ERROR(cudaDeviceSynchronize()); \
    HANDLE_ERROR(cudaGetLastError())
#ifdef CROSS_SYNC
#define CudaKernelRunAsync(a__, b__, c__, e__, ...) \
    a__<<<b__, c__>>>(__VA_ARGS__);                 \
    HANDLE_ERROR(cudaDeviceSynchronize());          \
    HANDLE_ERROR(cudaGetLastError());
#else
...
#define CudaStreamSynchronize(a__)            \
    HANDLE_ERROR(cudaStreamSynchronize(a__)); \
    HANDLE_ERROR(cudaGetLastError())
...
#define CudaStreamSynchronize(a__)           \
    HANDLE_ERROR(hipStreamSynchronize(a__)); \
    HANDLE_ERROR(hipGetLastError())
...
#define CudaFuncGetAttributes(a__, b__) \
    {                                   \
        a__->binaryVersion = 0;         \
        a__->constSizeBytes = -1;       \
        a__->localSizeBytes = -1;       \
        a__->maxThreadsPerBlock = 32;   \
        a__->numRegs = -1;              \
        a__->ptxVersion = -1;           \
        a__->sharedSizeBytes = -1;      \
    }
...
template <typename T>
inline void CudaAtomicAdd(T* sum, T val) {
#pragma omp atomic
    sum[0] += val;
}
template <typename T>
inline void CudaAtomicMax(T* sum, T val) {
#pragma omp critical
    {
...
template <typename T>
inline void CudaAtomicMaxReduceWarp(T* sum, T val) {
    CudaAtomicMax(sum, val);
}
