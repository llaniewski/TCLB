...
struct range_int {
    ...
    CudaDeviceFunction range_int<-C, -D, -A, -B> operator-() const { return -val; }
    template <int A_, int B_, int C_, int D_>
    ...
    CudaDeviceFunction static range_int<A, B, C, D> ensure(const int& x) { return x; }
};
