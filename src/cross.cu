...
void memcpy2D(void* dst_, int dpitch, const void* src_, int spitch, int width, int height) {
    ...
    for (int i=0; i<height; i++) { memcpy(dst + i*dpitch, src + i*spitch, width); }
}
...
struct ptrpair {
    ...
    size_t size = 0;
    ptrpair() = default;
    ptrpair(void** ptr_, size_t size_) {
        ptr = ptr_;
        size = size_;
    }
    bool operator<(const ptrpair& B) const { return size < B.size; }
};
...
CudaError cudaAllocFreeAll() {
    while (!freelist.empty()) {
        ...
        for (const auto& ptr_pair : ptr_list.second) *ptr_pair.ptr = nullptr;
        freelist.pop_back();
    }
    return CudaSuccess;
}
