...
int conControl::get(Context& cont, const char* svar, double scale, std::vector<double>& fill) {
    Context::iterator it;
    it = cont.begin();
    if (it->second.size() != fill.size()) { fill.resize(it->second.size()); }
    for (size_t i = 0; i < it->second.size(); i++) { fill[i] = 0; }
    ...
    while (std::getline(s, var, '+')) {
        ...
        if (it == cont.end()) {
            ...
            for (size_t i = 0; i < fill.size(); i++) { fill[i] += nscale*scale; }
        } else {
            output("Getting %s from context with a scale of %lf (*%lf)\n", it->first.c_str(), nscale, scale);
            for (size_t i = 0; i < fill.size(); i++) { fill[i] += it->second[i]*nscale*scale; }
        }
        sum_i++;
    }
    return 0;
...
int conControl::Internal(pugi::xml_node n) {
    ...
    }
    {
        ...
        }
        {
            ...
            csv_data["_time"];
            if (get(csv_data, time_str, time_scale, csv_data["_time"])) { return -1; }
        }
        int k = 0;
...
int conControl::Init() {
    ...
    std::vector<double> s;
    for (int i=0; i<iter; i++) { s.push_back(sin(i/500.)*0.01); }
