...
int acESYSParticle::Init() {
    ...
    if (need_file) {
        ...
        int nx = 1, ny = 1, nz = 1;
        {
            ...
            for (int nx1=1; nx1<=nx0; nx1++)
                if (nx0 % nx1 == 0) {
                    for (int ny1=1; ny1<=ny0; ny1++)
                        if (ny0 % ny1 == 0) {
                            for (int nz1=1; nz1<=nz0; nz1++)
                                if (nz0 % nz1 == 0) {
        ...
        if (D_MPI_RANK == 0) {
            ...
            fprintf(f,
                    "%s.initNeighbourSearch( particleType=\"%s\", gridSpacing=%lg, verletDist=%lg )\n",
                    sim.c_str(),
                    particle_type.c_str(),
                    gridSpacing,
                    verletDist);
            fprintf(f,
                    "%s.setSpatialDomain( BoundingBox(Vec3(%lg,%lg,%lg), Vec3(%lg,%lg,%lg)), circDimList = [%s, %s, %s])\n",  // JM [%s,False, False]
                    sim.c_str(),
                    0.0,
                    0.0,
                    0.0,
            ...
            fprintf(
                f, "%s.createInteractionGroup(	RemoteForcePrms(name=\"tclb\", remote_name=\"%s\", max_rad=%lg) )\n", sim.c_str(), MPMD.name.c_str(), maxRad);
