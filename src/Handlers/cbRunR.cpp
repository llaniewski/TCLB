...
template <typename T>
Rcpp::IntegerVector SingleInteger(T i) {
...
public:
    ...
    virtual SEXP Call(Rcpp::List) {
        ERROR("R: Called a non-callable rWrapper");
        return rNull;
    }
...
public:
    ...
    SEXP Dollar(std::string name) { return Rcpp::NumericVector(0.0); }
    void DollarAssign(std::string zone, SEXP v_) {
        ...
        if (zone_iter != solver->setting_zones.end()) zone_number = zone_iter->second;
        else {
    ...
    Rcpp::CharacterVector Names() {
        ...
        for (const auto& [name, id] : solver->setting_zones) { ret.push_back(name); }
        return ret;
    }
};
...
public:
    ...
    void DollarAssign(std::string name, SEXP v_) {
        ...
        if (zset) { ERROR("R: ZoneSetting not supported in rSetting"); }
    ...
    Rcpp::CharacterVector Names() {
        Rcpp::CharacterVector ret;
        for (const Model::Setting& it : solver->lattice->model->settings) { ret.push_back(it.name); }
        for (const Model::ZoneSetting& it : solver->lattice->model->zonesettings) { ret.push_back(it.name); }
        return ret;
    }
};
...
public:
    ...
    Rcpp::CharacterVector Names() {
        Rcpp::CharacterVector ret;
        for (const Model::Field& it : solver->lattice->model->fields) { ret.push_back(it.name); }
        return ret;
    }
};
...
public:
    ...
    SEXP Dollar(std::string name) {
        ...
        Rcpp::NumericVector ret(tmp.begin(), tmp.end());
        std::vector<int> retdim = solver->lattice->shape();
        if (comp != 1) { retdim.insert(retdim.begin(), 1, comp); }
        ret.attr("dim") = Rcpp::IntegerVector(retdim.begin(), retdim.end());
        return ret;
    }
    Rcpp::CharacterVector Names() {
...
public:
    SEXP Dollar(std::string name) { return rWrap(new rAction(name.c_str())); }
    Rcpp::CharacterVector Names() {
        Rcpp::CharacterVector ret;
        for (const Model::Action& it : solver->lattice->model->actions) { ret.push_back(it.name); }
        return ret;
    }
};
...
public:
    ...
    void DollarAssign(std::string name, SEXP v_) {
        ...
        if (it) {
            ...
            if (some_na) { ERROR("Some NA in Geometry (%s) assignment", it.name.c_str()); }
    ...
    SEXP Dollar(std::string name) {
        ...
        if (it) {  // Geometry components
            ...
            for (size_t i=0; i<size; i++) { small[i] = 1 + ((NodeType[i] & it.flag) >> it.shift); }
    ...
    virtual Rcpp::CharacterVector Names() {
        ...
        ret.push_back("size");
        ret.push_back("dim");
        for (const Model::NodeTypeGroupFlag& it : solver->lattice->model->nodetypegroupflags) { ret.push_back(it.name); }
        return ret;
    }
};
...
void initializePy() {
    ...
    R.parseEval("py$Solver = r_to_py(Solver)");
}
