...
public:
    ...
    struct Global : UnitThing {
        ...
        inline Global(const int& id_, const std::string& name_, const std::string& unit_,
                const MPI_Op& operation_, const int& inObjId_, const bool& isAdjoint_=false)
            : UnitThing(id_,name_,unit_), operation(operation_), inObjId(inObjId_), isAdjoint(isAdjoint_) {}
    };
