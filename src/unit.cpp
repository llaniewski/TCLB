...
void GaussSolve(double A[], double b[], double x[], int n) {
    ...
    for (k = 0; k < n - 1; k++) {
        ...
        for (i = k + 1; i < n; i++) {
            ...
            for (j=0; j<n; j++) { A[IND(i, j)] -= m*A[IND(k, j)]; }
            b[i] -= m*b[k];
        }
    }
    ...
    for (i = n - 1; i >= 0; i--) {
        double sum = 0.0;
        for (j = n - 1; j >= i + 1; j--) sum += A[IND(i, j)]*x[j];
...
UnitVal UnitEnv::readUnit(const std::string& val) const {
    ...
    while (val[i]) {
        ...
        l = i;
        p = 1;
        if (l - k > 0) { p = atof(val.substr(k, l - k).c_str()); }
        if (k - j > 0) {
...
void UnitEnv::makeGauge() {
    ...
    int i = 0, j;
    for (i = 0; i < m_unit*m_unit; i++) { Mat[i] = 0; }
    i = 0;
    for (auto el = gauge.begin(); el != gauge.end(); el++) {
        UnitVal v = el->second;
        for (j=0; j<m_unit; j++) { Mat[m_unit*j + i] = v.uni[j]; }
        b[i] = log(v.val);
        i++;
    }
    ...
    for (int j=0; j<m_unit; j++) { scale[j] = exp(-x[j]); }
}
void UnitEnv::printGauge() const {
    ...
    output("------------------------------------------\n");
    for (int j=0; j<m_unit; j++) { output("| 1 %s = %lf units\n", m_units[j].c_str(), scale[j]); }
    output("\\-----------------------------------------\n");
}
