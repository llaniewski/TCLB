...
function try {
	...
	if env time --quiet -f "%e" -o $log.time "$@" >$log 2>&1; then
		RES=true
	else
		RES=false
	fi
	if test $NEG != $RES; then
...
if ! test -f "CLB/$MODEL/main"; then
	echo "\"$MODEL\" is not compiled"
	echo "  run: make $MODEL"
	exit -1
fi

if ! test -f "tests/external/README.md"; then
...
if ! test -d "tests/external/$MODEL"; then
...
TESTS_ARG="$*"
if test -z "$TESTS_ARG"; then
	TESTS_ARG="."
fi

TESTS=""
DIR="tests/external/$MODEL"
for t in $TESTS_ARG; do
	ADD=""
	if test -f "$DIR/$t"; then
		ADD="$t"
	elif test -f "$DIR/$t.test"; then
		ADD="$t.test"
	elif test -d "$DIR/$t"; then
		ADD="$(
			cd $DIR
			ls $t/*.test 2>/dev/null
		)"
	else
...
function runline {
	...
	run)
		...
		;;
	fail)
		try "running solver (should fail)" '!' "$@"
		return $?
		;;
	csvconcatenate)
		try "concatenating csv files" $TCLB/tools/csvconcatenate "$@"
		return $?
		;;
	esac
	R=$1
	...
	case $CMD in
	exists)
		try "checking $R (exists)" test -f "$R"
		return $?
		;;
	sha1) G="$R.sha1" ;;
	*) G="$R" ;;
	esac
	if test -f "$TEST_DIR/$1"; then
		G="$1"
		shift
	fi
	G="$TEST_DIR/$G"
	...
	case $CMD in
	need)
		try "copy needed file" cp "$G" "$R"
		return $?
		;;
	csvdiff)
		try "checking $R (csvdiff)" $TCLB/tools/csvdiff -a "$R" -b "$G" -x "${1:-1e-10}" -d ${2:-$CSV_DISCARD}
		return $?
		;;
	diff)
	...
	sha1)
		...
		;;
	pvtidiff)
		try "checking $R (pvtidiff)" $TCLB/CLB/$MODEL/compare "$R" "$G" "${1:-8}" ${2:-} ${3:-} ${4:-}
		return $?
		;; # ${2:-8} is { if $2 == "" then "8" else $2 }
	*)
		echo "unknown: $CMD"
		return -1
		;;
...
function testModel {
	for t in $TESTS; do
		...
		if test -f "$EXC_SH"; then
			source "$EXC_SH"
		fi
		if test -f "tests/external/$MODEL/$t"; then
			echo -e "\n\e[1mRunning $TEST test...\e[0m"
			mkdir -p $TDIR
			while read -r -u 3 line; do
				if ! (cd $TDIR && runline $(eval echo $line)); then
					RESULT="FAILED"
					break
				fi
...
REPEAT=1
while test "$REPEAT" -le "$REPEATS"; do
	test "$REPEATS" -gt "1" && echo "############ repeat: $REPEAT ################"
	testModel $REPEAT
	REPEAT=$(expr $REPEAT + 1)
done

if test "x$GLOBAL" == "xOK"; then
	exit 0
else
